<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">{{ __('Edit Role') }}: {{ $role->name }}</h2>
    </x-slot>
                ...
                <form method="POST" action="{{ route('roles.update', $role) }}">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $role->name) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            ...
                            <div class="grid grid-cols-2 gap-2 max-h-64 overflow-y-auto border border-gray-200 rounded-md p-4">
                                @foreach($permissions as $group => $groupPermissions)
                                    <div class="col-span-2">
                                        <p class="mb-2 mt-2 text-xs font-semibold uppercase tracking-wide text-gray-500">{{ __($group) }}</p>
                    ...
                    <div class="mt-6 flex items-center justify-end gap-3">
                        ...
                        <button type="submit" class="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">{{ __('Update Role') }}</button>
                    </div>
                </form>
            </div>
        </div>
