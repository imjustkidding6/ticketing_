...
@php
    ...
    $tutorials = \App\Http\Controllers\TutorialController::TUTORIALS;
...
<div class="help-portal-container flex flex-col gap-8 pb-16">
    <!-- Header Area -->
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-[var(--border-soft)]">
        <div>
            <h1 class="page-title text-[var(--text-primary)]">Help & Tutorials</h1>
        ...
        <div class="flex-shrink-0">
            <a href="{{ route('admin.help.download-manual') }}" download class="btn-download-pill">
    ...
    <div class="flex flex-col lg:flex-row gap-8 items-start">
        ...
        <div x-data="{ sidebarCollapsed: true }" class="lg:hidden w-full">
            ...
            <div x-show="!sidebarCollapsed" x-collapse class="mt-2 bg-[var(--bg-card)] p-4 rounded-xl border border-[var(--border-soft)] shadow-md flex flex-col gap-2">
                @foreach($tutorials as $navSlug => $navTutorial)
                    <a href="{{ route('admin.help.show', $navSlug) }}" class="help-nav-link {{ $navSlug === $slug ? 'active' : '' }}">
        ...
        <aside class="hidden lg:block lg:w-[22%] shrink-0">
            <div class="bg-[var(--bg-card)] p-5 rounded-2xl border border-[var(--border-soft)] shadow-[var(--shadow)] flex flex-col gap-4">
                <nav class="flex flex-col gap-1" x-data="{
                    ...
                    @foreach($tutorials as $navSlug => $navTutorial)
                        ...
                        @endphp
                        <a href="{{ route('admin.help.show', $navSlug) }}"
        ...
        <div class="w-full lg:w-[78%] flex flex-col gap-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-6 border-t border-[var(--border-soft)]">
                <!-- Previous Button -->
                @if($prevSlug)
                    <a href="{{ route('admin.help.show', $prevSlug) }}" class="nav-btn items-start">
                ...
                <!-- Next Button -->
                @if($nextSlug)
                    <a href="{{ route('admin.help.show', $nextSlug) }}" class="nav-btn items-end text-right">
            ...
            <div class="border-t border-[var(--border-soft)] pt-12 mt-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    @foreach($relatedTutorials as $relSlug => $relTutorial)
                        <a href="{{ route('admin.help.show', $relSlug) }}" class="related-card p-5 flex flex-col justify-between h-full hover:-translate-y-1">
                            <div>
