<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">{{ __('Create SLA Policy') }}</h2>
    </x-slot>
            ...
            <div class="overflow-hidden rounded-xl bg-white p-6 shadow-sm">
                <form method="POST" action="{{ route('sla.store') }}">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" name="response_time_hours" id="response_time_hours" value="{{ old('response_time_hours', 4) }}" min="1" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                @error('response_time_hours') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label for="resolution_time_hours" class="block text-sm font-medium text-gray-700">{{ __('Resolution Time (hours)') }}</label>
                                <input type="number" name="resolution_time_hours" id="resolution_time_hours" value="{{ old('resolution_time_hours', 24) }}" min="1" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                @error('resolution_time_hours') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                            </div>
                        </div>
                    ...
                    <div class="mt-6 flex items-center justify-end gap-3">
                        ...
                        <button type="submit" class="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">{{ __('Create Policy') }}</button>
                    </div>
                </form>
            </div>
        </div>
