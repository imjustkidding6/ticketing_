...
@section('content')
    ...
    <div class="sla-edit-container animate-fade-in space-y-6" x-data="{
        rows: {
            critical: {
        ...
        <div class="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-800">
            <div class="space-y-1">
                ...
                <div class="flex items-center gap-2.5 mt-2">
                    ...
                    <h2 class="text-xl font-extrabold text-[var(--text-primary)] tracking-tight">Configure SLA Policies</h2>
                </div>
            </div>
        </div>
        ...
        <form method="POST" action="{{ request()->routeIs('admin.*') ? route('admin.sla.update-tier', $tier) : route('sla.update-tier', $tier) }}" @submit="submitForm($event)" class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start pb-24">
            ...
            <div class="lg:col-span-2 space-y-6">
                @foreach(['critical', 'high', 'medium', 'low'] as $priority)
                    ...
                    <div class="premium-card p-6 priority-card-{{ $priority }} space-y-4" 
                        ...
                        <div class="flex items-start justify-between">
                            <div class="space-y-1">
                                <div class="flex items-center gap-2">
                                    ...
                                    <h3 class="text-sm font-bold text-[var(--text-primary)] capitalize">{{ $priority }} Severity Target</h3>
                                </div>
                                <p class="text-xs text-[var(--text-secondary)] leading-relaxed pr-6">{{ $description }}</p>
                            </div>
                            <div class="flex items-center gap-2.5 shrink-0">
                                <span class="text-xs font-semibold text-[var(--text-secondary)]" x-text="rows.{{ $priority }}.is_active ? 'Active' : 'Paused'"></span>
                        ...
                        <!-- Target inputs -->
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-3 border-t border-slate-100 dark:border-slate-800/60" x-show="rows.{{ $priority }}.is_active" x-collapse>
                            <!-- Response Time Target -->
                            <div class="space-y-1.5">
                                <div class="flex items-center justify-between">
                                    <label class="text-xs font-bold text-[var(--text-secondary)]">Response Target (Hours)</label>
                                    <span class="text-[10px] text-[var(--text-secondary)] font-medium">Default: {{ $defaultValues[0] }}h</span>
                                </div>
                                <div class="relative rounded-xl shadow-sm">
                                    <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                                        <svg class="h-4.5 w-4.5 text-[var(--text-secondary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                    </div>
                                    <input type="number" min="1" step="1" required
                                           ...
                                           :class="validationErrors['{{ $priority }}_response'] ? 'border-rose-500 focus:border-rose-500 focus:ring-rose-200' : 'border-[var(--border-soft)] focus:border-indigo-500 focus:ring-indigo-200'">
                                </div>
                            ...
                            <div class="space-y-1.5">
                                <div class="flex items-center justify-between">
                                    <label class="text-xs font-bold text-[var(--text-secondary)]">Resolution Target (Hours)</label>
                                    <span class="text-[10px] text-[var(--text-secondary)] font-medium">Default: {{ $defaultValues[1] }}h</span>
                                </div>
                                <div class="relative rounded-xl shadow-sm">
                                    <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                                        <svg class="h-4.5 w-4.5 text-[var(--text-secondary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z" /></svg>
                                    </div>
                                    <input type="number" min="1" step="1" required
                                           name="rows[{{ $priority }}][resolution]"
                                           x-model="rows.{{ $priority }}.resolution"
                                           class="input-number-sla w-full pl-10 pr-4 text-left focus:ring-2"
                                           :class="validationErrors['{{ $priority }}_resolution'] || validationErrors['{{ $priority }}_resolution_compare'] ? 'border-rose-500 focus:border-rose-500 focus:ring-rose-200' : 'border-[var(--border-soft)] focus:border-indigo-500 focus:ring-indigo-200'">
                                </div>
                        ...
                        <!-- Inactive Alert -->
                        <div x-show="!rows.{{ $priority }}.is_active" class="p-3 bg-slate-50 dark:bg-slate-800/40 border border-slate-200/10 rounded-xl text-xs text-[var(--text-secondary)] flex items-center gap-2">
                            <svg class="h-4 w-4 text-[var(--text-secondary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M10 9v6m4-6v6m7-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
            ...
            <div class="lg:sticky lg:top-6 space-y-6">
                <div class="premium-card p-6 space-y-6">
                    <div class="flex items-center gap-2.5 pb-4 border-b border-slate-150 dark:border-slate-800">
                        <svg class="h-5 w-5 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>
                        <h3 class="text-sm font-extrabold text-[var(--text-primary)] tracking-tight">Live Profile Performance</h3>
                    </div>

                    <!-- Gauge Mockup / Score -->
                    <div class="flex flex-col items-center py-4 bg-slate-50/50 dark:bg-slate-800/10 rounded-2xl border border-slate-100 dark:border-slate-800/30">
                        <span class="text-xs text-[var(--text-secondary)] font-bold uppercase tracking-wider">Estimated compliance</span>
                        <span class="text-4xl font-black mt-2 text-emerald-500 tracking-tight" x-text="liveStats.compliance + '%'"></span>
                        <span class="text-[10px] text-[var(--text-secondary)] mt-1 block">Based on resolution targets average</span>
                    </div>

                    <!-- Stats breakdown -->
                    <div class="space-y-3.5 text-xs font-semibold text-[var(--text-secondary)]">
                        <div class="flex justify-between border-b border-slate-100 dark:border-slate-800 pb-2.5">
                            <span>Fastest Response</span>
                            <span class="text-[var(--text-primary)]" x-text="liveStats.minResp + ' hours'"></span>
                        </div>
                        <div class="flex justify-between border-b border-slate-100 dark:border-slate-800 pb-2.5">
                            <span>Slowest Response</span>
                            <span class="text-[var(--text-primary)]" x-text="liveStats.maxResp + ' hours'"></span>
                        </div>
                        <div class="flex justify-between border-b border-slate-100 dark:border-slate-800 pb-2.5">
                            <span>Average Response</span>
                            <span class="text-[var(--text-primary)]" x-text="liveStats.avgResp + ' hours'"></span>
                        </div>
                        <div class="flex justify-between border-b border-slate-100 dark:border-slate-800 pb-2.5">
                            <span>Fastest Resolution</span>
                            <span class="text-[var(--text-primary)]" x-text="liveStats.minRes + ' hours'"></span>
                        </div>
                        <div class="flex justify-between border-b border-slate-100 dark:border-slate-800 pb-2.5">
                            <span>Slowest Resolution</span>
                            <span class="text-[var(--text-primary)]" x-text="liveStats.maxRes + ' hours'"></span>
                        </div>
                        <div class="flex justify-between pb-1">
                            <span>Average Resolution</span>
                            <span class="text-[var(--text-primary)]" x-text="liveStats.avgRes + ' hours'"></span>
                        </div>
                    </div>
            ...
            <div class="floating-action-bar" x-show="isDirty" x-cloak>
                <div class="p-4 bg-[var(--bg-card)]/95 backdrop-blur-md rounded-2xl shadow-2xl border border-[var(--border-soft)] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="text-left">
                            <span class="block text-sm font-bold text-[var(--text-primary)]">Unsaved configuration changes</span>
                            <span class="block text-xs text-[var(--text-secondary)]">Save policies or reset targets.</span>
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                        <button type="button" @click="showResetModal = true" class="px-4 py-2 text-xs font-bold rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-[var(--text-primary)] border border-[var(--border-soft)] transition cursor-pointer">
                            Reset Changes
                        </button>
                        <button type="submit" :disabled="hasErrors" class="px-5 py-2 text-xs font-bold rounded-xl bg-[#5B5FF6] hover:bg-[#4F46E5] text-white shadow-sm disabled:opacity-40 disabled:cursor-not-allowed transition border-none cursor-pointer">
                            Save Policies
                        </button>
                    </div>
                </div>
        ...
        <template x-if="showResetModal">
            <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
                <div class="bg-[var(--bg-card)] rounded-2xl max-w-md w-full p-6 shadow-2xl border border-[var(--border-soft)] space-y-5 animate-fade-in" @click.outside="showResetModal = false">
                    <div class="flex items-start gap-4">
                        ...
                        <div>
                            <h3 class="text-lg font-bold text-[var(--text-primary)]">Reset Configuration Changes?</h3>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
                        <button type="button" @click="showResetModal = false" class="px-4 py-2 text-sm font-semibold rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-[var(--text-primary)] transition cursor-pointer">
                            No, keep editing
                        </button>
        ...
        <template x-if="showDiscardModal">
            <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
                <div class="bg-[var(--bg-card)] rounded-2xl max-w-md w-full p-6 shadow-2xl border border-[var(--border-soft)] space-y-5 animate-fade-in" @click.outside="showDiscardModal = false">
                    <div class="flex items-start gap-4">
                        <div class="p-3.5 bg-rose-50 dark:bg-rose-950/20 text-rose-600 dark:text-rose-400 rounded-xl shrink-0">
                            <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
                        </div>
                        <div>
                            <h3 class="text-lg font-bold text-[var(--text-primary)]">Discard Unsaved Changes?</h3>
                            <p class="text-sm text-[var(--text-secondary)] mt-2 leading-relaxed">
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
                        <button type="button" @click="showDiscardModal = false; leaveTarget = '';" class="px-4 py-2 text-sm font-semibold rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-[var(--text-primary)] transition cursor-pointer">
                            No, keep editing
                        </button>
