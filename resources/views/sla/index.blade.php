<x-app-layout>
        ...
        Alpine.data('slaDashboard', () => ({
            ...
            init() {
                ...
                window.addEventListener('theme-changed', (e) => {
                    this.updateChartThemes(e.detail ? e.detail.darkMode : document.documentElement.classList.contains('dark'));
                });
    ...
    <div class="py-6 sla-dashboard animate-fade-in" x-data="slaDashboard">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <!-- Error Flash Alert -->
            @if(session('error'))
                <div class="flex items-center justify-between p-4 rounded-xl border border-rose-200/50 bg-rose-50/50 dark:bg-rose-950/20 dark:border-rose-800/30 text-rose-800 dark:text-rose-400 text-sm shadow-sm transition-all duration-300">
                    <div class="flex items-center gap-3">
                        <svg class="h-5 w-5 text-rose-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <span class="font-semibold">{{ session('error') }}</span>
                    </div>
                    <button @click="$el.parentElement.remove()" class="text-rose-400 hover:text-rose-600 transition">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                    </button>
                </div>
            @endif
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-gray-200 dark:border-gray-800 no-print">
                ...
                <div class="flex items-center flex-wrap gap-3">
                    <button @click="openCreateModal()" class="inline-flex items-center gap-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2.5 text-sm font-semibold shadow-sm transition cursor-pointer border-none">
                    ...
                    <button @click="showSeedModal = true" class="inline-flex items-center gap-2 rounded-xl bg-gray-900 hover:bg-black text-white dark:bg-gray-800 dark:hover:bg-gray-700 px-4 py-2.5 text-sm font-semibold shadow-sm transition cursor-pointer">
            ...
            <div class="flex border-b border-gray-200 dark:border-gray-800 no-print">
                <button @click="activeTab = 'dashboard'" :class="activeTab === 'dashboard' ? 'border-indigo-600 text-indigo-600 dark:text-indigo-400 font-bold border-b-2' : 'text-gray-500 hover:text-gray-800'" class="px-6 py-3 text-sm font-semibold transition focus:outline-none flex items-center gap-2">
                    ...
                    <span>📊 SLA Health & Command Center</span>
                </button>
                <button @click="activeTab = 'policies'" :class="activeTab === 'policies' ? 'border-indigo-600 text-indigo-600 dark:text-indigo-400 font-bold border-b-2' : 'text-gray-500 hover:text-gray-800'" class="px-6 py-3 text-sm font-semibold transition focus:outline-none flex items-center gap-2">
                    ...
                    <span>📋 SLA Policy Registry (CRUD)</span>
                </button>
                <button @click="activeTab = 'tiers'" :class="activeTab === 'tiers' ? 'border-indigo-600 text-indigo-600 dark:text-indigo-400 font-bold border-b-2' : 'text-gray-500 hover:text-gray-800'" class="px-6 py-3 text-sm font-semibold transition focus:outline-none flex items-center gap-2">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 gap-4">
                    <div class="premium-card p-4 flex flex-col justify-between">
                        ...
                        <div class="flex items-baseline gap-1 mt-1">
                            <span class="text-2xl font-extrabold text-gray-900 dark:text-gray-100 tracking-tight">{{ $healthStats['totalPolicies'] }}</span>
                            <span class="text-[10px] text-gray-400">defined</span>
                        </div>
                    </div>
                    <div class="premium-card p-4 flex flex-col justify-between">
                        <span class="text-[10px] font-extrabold uppercase tracking-wider text-gray-400">Active Policies</span>
                        <div class="flex items-baseline gap-1 mt-1">
                    ...
                    <div class="premium-card p-4 flex flex-col justify-between">
                        <span class="text-[10px] font-extrabold uppercase tracking-wider text-gray-400">Avg Response Target</span>
                        <div class="flex items-baseline gap-1 mt-1">
                    ...
                    <div class="premium-card p-4 flex flex-col justify-between">
                        <span class="text-[10px] font-extrabold uppercase tracking-wider text-gray-400">Avg Resolution Target</span>
                        <div class="flex items-baseline gap-1 mt-1">
                    ...
                    <div class="premium-card p-4 flex flex-col justify-between">
                        <span class="text-[10px] font-extrabold uppercase tracking-wider text-gray-400">Most Used Policy</span>
                        <div class="mt-1 min-w-0">
                            <span class="block text-sm font-bold text-gray-900 dark:text-gray-100 truncate">{{ $healthStats['mostUsedPolicy'] }}</span>
                            <span class="text-[10px] text-indigo-500 font-bold">{{ $healthStats['mostUsedCount'] }} tickets</span>
                        </div>
                    </div>
                    <div class="premium-card p-4 flex flex-col justify-between text-rose-600 dark:text-rose-400">
                        <span class="text-[10px] font-extrabold uppercase tracking-wider text-gray-400">Overdue Violations</span>
                        <div class="flex items-baseline gap-1 mt-1">
                            <span class="text-2xl font-extrabold tracking-tight">{{ $overdueTickets->count() }}</span>
                            <span class="text-[10px] font-bold">open</span>
                        </div>
                    </div>
                    <div class="premium-card p-4 flex flex-col justify-between text-amber-500">
                        <span class="text-[10px] font-extrabold uppercase tracking-wider text-gray-400">Near Breach (<4h)</span>
                        <div class="flex items-baseline gap-1 mt-1">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="premium-card p-6 space-y-6 lg:col-span-2">
                        <div class="flex items-center justify-between">
                            <div>
                                <h3 class="text-base font-extrabold text-gray-900 dark:text-gray-100">Weekly SLA Compliance Progression</h3>
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Resolution compliance trend over the past 6 weeks</p>
                            </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-gray-100 dark:border-gray-800">
                            <div>
                                <h4 class="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">Response Speed Trend</h4>
                                <div id="chart-times-trend" class="w-full"></div>
                            </div>
                            <div>
                                <h4 class="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">Priority Compliance Breakdown</h4>
                        ...
                        <div>
                            <div class="flex items-center justify-between pb-3 border-b border-gray-100 dark:border-gray-800">
                                <div class="flex items-center gap-2">
                                    ...
                                    <h3 class="text-base font-extrabold text-gray-900 dark:text-gray-100">Near Breach Radar</h3>
                                </div>
                            ...
                            <div class="mt-4 space-y-3 max-h-[420px] overflow-y-auto pr-1">
                                <template x-if="nearBreachList.length === 0">
                                    <div class="text-center py-10 space-y-2">
                                        ...
                                        <p class="text-sm font-semibold text-gray-700 dark:text-gray-300">All targets safe!</p>
                                        <p class="text-xs text-gray-400">No tickets are within 4 hours of SLA breach.</p>
                                    </div>
                                </template>

                                <template x-for="t in nearBreachList" :key="t.id">
                                    <div class="p-3.5 rounded-xl bg-gray-50 dark:bg-gray-800/40 border border-gray-100 dark:border-gray-800 space-y-2 transition hover:border-amber-300">
                                        ...
                                        <p class="text-xs font-medium text-gray-800 dark:text-gray-200 line-clamp-1" x-text="t.subject"></p>
                                        <div class="flex items-center justify-between text-[11px] text-gray-400 pt-1">
                                            <span x-text="'Client: ' + t.client"></span>
                                            <div class="flex items-center gap-2">
                                                <button @click="sendReminder(t.id)" class="text-indigo-600 dark:text-indigo-400 hover:underline font-semibold cursor-pointer">Notify</button>
                                                <span>•</span>
                                                <button @click="escalateTicket(t.id)" class="text-rose-600 dark:text-rose-400 hover:underline font-semibold cursor-pointer">Escalate</button>
                                            </div>
                                        </div>
                                    </div>
                                </template>
                ...
                <div class="premium-card p-5 space-y-4 no-print">
                    <div class="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
                        ...
                        <div class="relative flex-1 min-w-[240px]">
                            <svg class="absolute left-3.5 top-3 h-4 w-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                            <input type="text" x-model="searchQuery" placeholder="Search by policy name, tier, or description..." class="w-full search-input-sla" />
                        </div>

                        <!-- Dropdown Filters -->
                        <div class="flex items-center flex-wrap gap-3">
                            <select x-model="selectedTier" class="filter-select cursor-pointer">
                            ...
                            <select x-model="selectedPriority" class="filter-select cursor-pointer">
                            ...
                            <select x-model="selectedStatus" class="filter-select cursor-pointer">
                                <option value="All">All Statuses</option>
                                <option value="Active">Active Only</option>
                                <option value="Archived">Archived / Paused</option>
                            </select>

                            <button @click="clearFilters()" class="px-3 py-2 text-xs font-semibold text-gray-500 hover:text-gray-800 dark:hover:text-gray-200 transition cursor-pointer">
                                Clear Filters
                            </button>
                        </div>
                    </div>
                    ...
                    <!-- Bulk Actions Bar -->
                    <div x-show="selectedRows.length > 0" class="flex items-center justify-between bg-indigo-50 dark:bg-indigo-950/40 p-3 rounded-xl border border-indigo-200/50 dark:border-indigo-800/40 text-xs text-indigo-900 dark:text-indigo-300">
                        <div class="flex items-center gap-2 font-bold">
                            <span x-text="selectedRows.length + ' policies selected'"></span>
                        </div>
                        <div class="flex items-center gap-2">
                            <button @click="applyBulkAction('activate')" class="px-3 py-1.5 rounded-lg bg-emerald-600 text-white font-semibold hover:bg-emerald-700 cursor-pointer">Activate Selected</button>
                            <button @click="applyBulkAction('deactivate')" class="px-3 py-1.5 rounded-lg bg-amber-600 text-white font-semibold hover:bg-amber-700 cursor-pointer">Archive Selected</button>
                            <button @click="applyBulkAction('delete')" class="px-3 py-1.5 rounded-lg bg-rose-600 text-white font-semibold hover:bg-rose-700 cursor-pointer">Delete Selected (Safe)</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="premium-card overflow-hidden">
                    ...
                    <div class="overflow-x-auto sticky-table-container">
                        <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-800 text-left text-sm">
                            <thead class="bg-gray-50/70 dark:bg-gray-800/50 text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 no-print">
                                <tr>
                                    <th class="px-4 py-3.5 w-10 text-center">
                                        <input type="checkbox" @click="toggleSelectAll()" :checked="selectedRows.length === paginatedPolicies.length && paginatedPolicies.length > 0" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer" />
                                    </th>
                                    <th class="px-6 py-3.5 cursor-pointer hover:text-gray-900 dark:hover:text-gray-100" @click="toggleSort('name')">
                                        Policy Name <span x-show="sortBy === 'name'" x-text="sortDesc ? '↓' : '↑'"></span>
                                    </th>
                                    <th class="px-4 py-3.5 cursor-pointer hover:text-gray-900 dark:hover:text-gray-100" @click="toggleSort('client_tier')">
                                        Tier <span x-show="sortBy === 'client_tier'" x-text="sortDesc ? '↓' : '↑'"></span>
                                    </th>
                                    <th class="px-4 py-3.5 cursor-pointer hover:text-gray-900 dark:hover:text-gray-100" @click="toggleSort('priority')">
                                        Priority <span x-show="sortBy === 'priority'" x-text="sortDesc ? '↓' : '↑'"></span>
                                    </th>
                                    <th class="px-4 py-3.5 text-center cursor-pointer hover:text-gray-900 dark:hover:text-gray-100" @click="toggleSort('response_time_hours')">
                                        Response Target <span x-show="sortBy === 'response_time_hours'" x-text="sortDesc ? '↓' : '↑'"></span>
                                    </th>
                                    <th class="px-4 py-3.5 text-center cursor-pointer hover:text-gray-900 dark:hover:text-gray-100" @click="toggleSort('resolution_time_hours')">
                                ...
                                <template x-for="p in paginatedPolicies" :key="p.id">
                                    <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition">
                                        <td class="px-4 py-4 text-center no-print">
                                            <input type="checkbox" :value="p.id" x-model="selectedRows" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer" />
                                        </td>
                                        <td class="px-6 py-4 font-bold text-gray-900 dark:text-gray-100">
                                            <span x-text="p.name"></span>
                                            <div class="text-[11px] text-gray-400 font-normal mt-0.5 line-clamp-1" x-text="p.description ? p.description : 'Standard SLA target specification configuration.'"></div>
                                        </td>
                                        <td class="px-4 py-4">
                                            <span class="px-2.5 py-0.5 rounded text-[10px] font-extrabold uppercase tracking-wider bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300" x-text="p.client_tier"></span>
                                        </td>
                                        <td class="px-4 py-4">
                                            <span class="sla-pill" :class="'sla-badge-' + p.priority" x-text="p.priority"></span>
                                        </td>
                                        <td class="px-4 py-4 text-center font-bold text-gray-900 dark:text-gray-100" x-text="p.response_time_hours + ' Hours'"></td>
                                        <td class="px-4 py-4 text-center font-bold text-gray-900 dark:text-gray-100" x-text="p.resolution_time_hours + ' Hours'"></td>
                                        <td class="px-4 py-4 text-center">
                                            ...
                                            <span x-show="!p.is_active" class="inline-flex items-center rounded-full bg-gray-100 dark:bg-gray-800 px-2.5 py-0.5 text-xs font-semibold text-gray-500 dark:text-gray-400">Archived</span>
                                        </td>
                                        <!-- Policy Usage Badge -->
                                        <td class="px-4 py-4 text-center">
                                            <div class="flex items-center justify-center gap-1.5 text-xs">
                                                ...
                                                <a :href="'/tickets?sla_policy_id=' + p.id + '&status=closed'" title="Closed Tickets" class="px-2 py-0.5 rounded bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400 font-semibold hover:underline" x-text="(p.closed_tickets_count || 0) + ' closed'"></a>
                                        ...
                                        <td class="px-6 py-4 text-right no-print" x-data="{ open: false }">
                                            <div class="relative inline-block text-left">
                                                <button @click="open = !open" @click.outside="open = false" type="button" class="p-2 rounded-lg bg-gray-50 hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300 transition cursor-pointer">
                                                ...
                                                <div x-show="open" class="absolute right-0 z-30 mt-2 w-44 origin-top-right rounded-xl bg-white dark:bg-gray-850 shadow-xl border border-gray-100 dark:border-gray-750 py-1 text-xs font-semibold text-gray-700 dark:text-gray-200 animate-fade-in" x-cloak>
                                                    <button @click="openViewModal(p); open = false;" class="w-full text-left px-4 py-2 hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center gap-2 cursor-pointer">
                                                        <svg class="h-3.5 w-3.5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                                        <span>View Details</span>
                                                    </button>
                                                    <button @click="openEditModal(p); open = false;" class="w-full text-left px-4 py-2 hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center gap-2 cursor-pointer">
                                                    ...
                                                    <button @click="openDuplicateModal(p); open = false;" class="w-full text-left px-4 py-2 hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center gap-2 cursor-pointer">
                                                    ...
                                                    <form method="POST" :action="p.toggle_url" class="inline">
                                                        @csrf
                                                        <button type="submit" class="w-full text-left px-4 py-2 hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center gap-2 cursor-pointer">
                                                    ...
                                                    <button @click="openDeleteModal(p); open = false;" class="w-full text-left px-4 py-2 text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/30 flex items-center gap-2 cursor-pointer">
                    ...
                    <!-- Table Pagination -->
                    <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between text-xs text-gray-500 no-print">
                        <span x-text="'Showing page ' + page + ' of ' + totalPages + ' (' + filteredPolicies.length + ' total policies)'"></span>
                        <div class="flex items-center gap-2">
                            <button @click="prevPage()" :disabled="page === 1" class="px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-750 disabled:opacity-40 cursor-pointer">Previous</button>
                            <button @click="nextPage()" :disabled="page === totalPages" class="px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-750 disabled:opacity-40 cursor-pointer">Next</button>
                        </div>
                    </div>
                </div>
                    ...
                    @foreach($tiers as $tier)
                        ...
                        <div class="premium-card p-6 flex flex-col justify-between space-y-6">
                            ...
                            <div class="mt-6 pt-4 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between gap-3 no-print">
                                <a href="{{ Route::has($routePrefix.'sla.edit-tier') ? route($routePrefix.'sla.edit-tier', ['tier' => $tier]) : '#' }}" class="inline-flex items-center justify-center flex-1 rounded-xl bg-gray-50 hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-250 py-2 text-sm font-semibold transition border border-gray-200 dark:border-gray-750">
        ...
        <template x-if="showCreateModal">
            <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm no-print">
                <div class="bg-white dark:bg-slate-900 rounded-2xl max-w-3xl w-full p-6 shadow-2xl border border-gray-100 dark:border-slate-800 space-y-6 animate-fade-in" @click.outside="showCreateModal = false">
                    
                    <!-- Modal Header -->
                    <div class="flex items-center justify-between pb-4 border-b border-gray-100 dark:border-gray-800">
                        <div>
                            <h3 class="text-lg font-bold text-gray-900 dark:text-gray-100" x-text="modalMode === 'edit' ? 'Edit SLA Policy' : (modalMode === 'duplicate' ? 'Duplicate SLA Policy' : 'Create SLA Policy')"></h3>
                            <p class="text-xs text-gray-500 dark:text-gray-400">Configure target SLA response and resolution times for tickets.</p>
                        </div>
                    ...
                    <form method="POST" :action="modalMode === 'edit' ? '/admin/sla/' + form.id : '{{ Route::has($routePrefix.'sla.store') ? route($routePrefix.'sla.store') : '/admin/sla' }}'">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-4 text-xs">
                                <div>
                                    <label class="block font-bold text-gray-700 dark:text-gray-300 mb-1">Policy Name *</label>
                                    <input type="text" name="name" x-model="form.name" required placeholder="e.g. Enterprise - Critical Priority" class="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-750 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-sm font-semibold" />
                                </div>

                                <div>
                                    <label class="block font-bold text-gray-700 dark:text-gray-300 mb-1">Description</label>
                                    <textarea name="description" x-model="form.description" rows="2" placeholder="Optional notes regarding target expectations..." class="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-750 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-xs"></textarea>
                                </div>

                                <div class="grid grid-cols-2 gap-3">
                                    <div>
                                        <label class="block font-bold text-gray-700 dark:text-gray-300 mb-1">Client Tier *</label>
                                        <select name="client_tier" x-model="form.client_tier" class="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-750 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-xs font-semibold cursor-pointer">
                                            <option value="basic">Basic Tier</option>
                                            <option value="premium">Premium Tier</option>
                                            <option value="enterprise">Enterprise Tier</option>
                                        </select>
                                    ...
                                    <div>
                                        <label class="block font-bold text-gray-700 dark:text-gray-300 mb-1">Ticket Priority *</label>
                                        <select name="priority" x-model="form.priority" class="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-750 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-xs font-semibold cursor-pointer">
                                            <option value="low">Low Priority</option>
                                            <option value="medium">Medium Priority</option>
                                            <option value="high">High Priority</option>
                                            <option value="critical">Critical Priority</option>
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    <div>
                                        <label class="block font-bold text-gray-700 dark:text-gray-300 mb-1">Response Target (Hours) *</label>
                                        <input type="number" name="response_time_hours" x-model="form.response_time_hours" min="0" required class="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-750 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-sm font-bold text-center" />
                                    </div>
                                    <div>
                                        <label class="block font-bold text-gray-700 dark:text-gray-300 mb-1">Resolution Target (Hours) *</label>
                                        <input type="number" name="resolution_time_hours" x-model="form.resolution_time_hours" min="1" required class="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-750 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-sm font-bold text-center" />
                                    </div>
                                </div>

                                <div class="flex items-center gap-2 pt-2">
                                    <input type="checkbox" name="is_active" id="is_active_input" value="1" :checked="form.is_active" @change="form.is_active = $el.checked" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer" />
                                    <label for="is_active_input" class="font-bold text-gray-700 dark:text-gray-300 cursor-pointer">Active Policy (Enforce SLA tracking)</label>
                                </div>
                            </div>

                            <!-- RIGHT COLUMN: Live Policy Preview Panel -->
                            <div class="bg-gray-50 dark:bg-gray-800/40 p-4 rounded-xl border border-gray-200/60 dark:border-gray-750 space-y-4">
                                <div class="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 pb-2">
                                    <span class="text-xs font-bold text-gray-500 uppercase tracking-wider">Live SLA Policy Preview</span>
                                    <span class="sla-pill" :class="'sla-badge-' + form.priority" x-text="form.priority"></span>
                                </div>

                                <div class="space-y-3 text-xs">
                                    <div class="p-3 bg-white dark:bg-gray-900 rounded-lg border border-gray-100 dark:border-gray-800 space-y-1">
                                        <span class="text-[10px] font-bold uppercase text-gray-400 block">Target Scope</span>
                                        <p class="font-extrabold text-gray-900 dark:text-gray-100" x-text="(form.client_tier || 'basic').toUpperCase() + ' Tier — ' + (form.priority || 'medium').toUpperCase() + ' Priority'"></p>
                                    </div>

                                    <div class="p-3 bg-white dark:bg-gray-900 rounded-lg border border-gray-100 dark:border-gray-800 space-y-1">
                                        <span class="text-[10px] font-bold uppercase text-emerald-600 dark:text-emerald-400 block">Response Target</span>
                                        <p class="font-bold text-gray-800 dark:text-gray-200" x-text="'Created + ' + (form.response_time_hours || 0) + ' Hours'"></p>
                                        <p class="text-[11px] text-gray-400 font-medium">e.g. Ticket created at 9:00 AM → First Response due by <strong class="text-gray-700 dark:text-gray-300" x-text="(form.response_time_hours || 0) + ' hours later'"></strong></p>
                                    </div>

                                    <div class="p-3 bg-white dark:bg-gray-900 rounded-lg border border-gray-100 dark:border-gray-800 space-y-1">
                                        <span class="text-[10px] font-bold uppercase text-indigo-600 dark:text-indigo-400 block">Resolution Target</span>
                                        <p class="font-bold text-gray-800 dark:text-gray-200" x-text="'Created + ' + (form.resolution_time_hours || 0) + ' Hours'"></p>
                                        <p class="text-[11px] text-gray-400 font-medium">e.g. Ticket created at 9:00 AM → Full Resolution due by <strong class="text-gray-700 dark:text-gray-300" x-text="(form.resolution_time_hours || 0) + ' hours later'"></strong></p>
                                    </div>
                                </div>
                        ...
                        <div class="flex justify-end gap-3 pt-6 border-t border-gray-100 dark:border-slate-800 mt-6">
                            <button type="button" @click="showCreateModal = false" class="px-4 py-2 text-sm font-semibold rounded-xl bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 transition cursor-pointer">
                                Cancel
                            </button>
                            <button type="submit" :disabled="validationErrors.length > 0 || (duplicateMatch && !form.overwrite)" class="px-5 py-2 text-sm font-semibold rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm transition cursor-pointer border-none disabled:opacity-40">
                                Save SLA Policy
                            </button>
                        </div>
                    </form>
        ...
        <template x-if="showViewModal && selectedPolicyForView">
            <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm no-print">
                <div class="bg-white dark:bg-slate-900 rounded-2xl max-w-md w-full p-6 shadow-2xl border border-gray-100 dark:border-slate-800 space-y-5 animate-fade-in" @click.outside="showViewModal = false">
                    <div class="flex items-center justify-between pb-3 border-b border-gray-100 dark:border-gray-800">
                        <div>
                            <h3 class="text-base font-bold text-gray-900 dark:text-gray-100" x-text="selectedPolicyForView.name"></h3>
                    ...
                    <div class="space-y-3 text-xs">
                        <div class="flex justify-between py-1.5 border-b border-gray-100 dark:border-gray-800">
                            <span class="text-gray-500">Client Tier:</span>
                            <span class="font-bold text-gray-900 dark:text-gray-100 uppercase" x-text="selectedPolicyForView.client_tier"></span>
                        </div>
                        <div class="flex justify-between py-1.5 border-b border-gray-100 dark:border-gray-800">
                            <span class="text-gray-500">Response Target:</span>
                        ...
                        <div class="flex justify-between py-1.5 border-b border-gray-100 dark:border-gray-800">
                            <span class="text-gray-500">Active Status:</span>
                            <span class="font-bold" :class="selectedPolicyForView.is_active ? 'text-emerald-500' : 'text-gray-400'" x-text="selectedPolicyForView.is_active ? 'Active' : 'Archived'"></span>
                        </div>
                        <div class="flex justify-between py-1.5 border-b border-gray-100 dark:border-gray-800">
                            <span class="text-gray-500">Total Assigned Tickets:</span>
                            <span class="font-bold text-gray-900 dark:text-gray-100" x-text="selectedPolicyForView.total_tickets_count || 0"></span>
                        </div>
                        <div class="flex justify-between py-1.5 border-b border-gray-100 dark:border-gray-800">
                            <span class="text-gray-500">Breached Tickets:</span>
                        ...
                        <div class="pt-2 text-gray-500">
                            <span class="block text-[10px] uppercase font-bold tracking-wider text-gray-400 mb-1">Description</span>
                            <p class="text-xs text-gray-700 dark:text-gray-300 italic" x-text="selectedPolicyForView.description || 'No description specified.'"></p>
                        </div>
                    </div>

                    <div class="flex justify-end pt-4 border-t border-gray-100 dark:border-slate-800">
                        <button type="button" @click="showViewModal = false" class="px-5 py-2 text-sm font-semibold rounded-xl bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200 transition cursor-pointer">
                            Close
                        </button>
                    </div>
                </div>
        ...
        <template x-if="showDeleteModal && selectedPolicyForDelete">
            <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm no-print">
                <div class="bg-white dark:bg-slate-900 rounded-2xl max-w-md w-full p-6 shadow-2xl border border-gray-100 dark:border-slate-800 space-y-5 animate-fade-in" @click.outside="showDeleteModal = false">
                    ...
                    <div class="flex items-start gap-4">
                        ...
                        <div>
                            <h3 class="text-lg font-bold text-gray-900 dark:text-gray-100" x-text="(selectedPolicyForDelete.total_tickets_count || 0) > 0 ? 'Policy Cannot Be Deleted' : 'Delete SLA Policy?'"></h3>
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1" x-text="(selectedPolicyForDelete.total_tickets_count || 0) > 0 ? 'This SLA Policy is currently assigned to ' + selectedPolicyForDelete.total_tickets_count + ' ticket(s). Deletion is prevented to preserve historical SLA tracking.' : 'Are you sure you want to permanently delete \'' + selectedPolicyForDelete.name + '\'?'"></p>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-gray-100 dark:border-slate-800">
                        <button type="button" @click="showDeleteModal = false" class="px-4 py-2 text-sm font-semibold rounded-xl bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 text-gray-700 dark:text-gray-300 transition cursor-pointer">
                            Cancel
                        </button>
        ...
        <template x-if="showSeedModal">
            <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm no-print">
                <div class="bg-white dark:bg-slate-900 rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-gray-100 dark:border-slate-800 space-y-5 animate-fade-in" @click.outside="showSeedModal = false">
                    <div class="flex items-start gap-4">
                        ...
                        <div>
                            <h3 class="text-lg font-bold text-gray-900 dark:text-gray-100">Seed Standard SLA Policies</h3>
