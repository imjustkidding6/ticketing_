<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between w-full">
            <h2 class="text-xl font-semibold leading-tight text-gray-800">{{ __('Category Report') }}</h2>
            <a href="{{ route('reports.export.categories', $filters) }}" class="inline-flex items-center rounded-md border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50">
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        @forelse($categoryReport as $cat)
                            <tr>
                                ...
                                <td class="px-6 py-4 text-sm text-gray-500">{{ $cat['department'] }}</td>
                                <td class="px-6 py-4 text-right text-sm font-semibold text-gray-900">{{ $cat['total'] }}</td>
                                <td class="px-6 py-4 text-right text-sm text-blue-600">{{ $cat['open'] }}</td>
                                <td class="px-6 py-4 text-right text-sm text-green-600">{{ $cat['closed'] }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center text-sm text-gray-500">{{ __('No category data for this period.') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
