<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between w-full">
            <h2 class="text-xl font-semibold leading-tight text-gray-800">{{ __('Department Report') }}</h2>
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        @forelse($departmentReport as $dept)
                            <tr>
                                ...
                                <td class="px-6 py-4 text-right text-sm font-semibold text-gray-900">{{ $dept['total'] }}</td>
                                <td class="px-6 py-4 text-right text-sm text-blue-600">{{ $dept['open'] }}</td>
                                <td class="px-6 py-4 text-right text-sm text-green-600">{{ $dept['closed'] }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-12 text-center text-sm text-gray-500">{{ __('No department data for this period.') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
