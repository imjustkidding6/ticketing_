{{-- In-app AI assistant for logged-in team members. Per-user memory (server-keyed by user_id). --}}
    ...
    <div x-show="open"
        ...
        <div class="flex items-center justify-between bg-indigo-600 px-4 py-3 text-white">
            <div class="flex items-center gap-2">
                ...
                <span class="text-sm font-semibold">{{ __('AI Assistant') }}</span>
            </div>
...
<script>
    function appAiChat() {
        return {
            ...
            input: '',
            loading: false,
            endpoint: '{{ route('assistant.message') }}',
            historyEndpoint: '{{ route('assistant.history') }}',
            newEndpoint: '{{ route('assistant.new') }}',
            ...
            async loadHistory() {
                try {
                    const res = await fetch(this.historyEndpoint, { headers: { 'Accept': 'application/json' } });
            ...
            async newChat() {
                try { await fetch(this.newEndpoint, { method: 'POST', headers: { 'X-CSRF-TOKEN': this.csrf, 'Accept': 'application/json' } }); } catch (e) {}
                this.messages = [];
                this.input = '';
                this.greet();
                this.$nextTick(() => this.scrollToEnd());
            ...
            toggle() {
                ...
                if (this.open) this.$nextTick(() => this.scrollToEnd());
            },
