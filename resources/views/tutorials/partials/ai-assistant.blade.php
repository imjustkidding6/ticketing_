<div class="prose prose-sm max-w-none text-gray-700">
    <h2 class="text-lg font-semibold text-gray-900">{{ __('AI Assistant') }}</h2>
    ...
    <div class="not-prose my-5 flex items-start gap-3 rounded-xl bg-gradient-to-br from-indigo-50 to-violet-50 p-4 ring-1 ring-indigo-100">
        ...
        <div class="text-sm text-indigo-900">
            ...
            <p class="mt-0.5 text-indigo-700">{{ __('Press the sparkle button, ask a question the way you would ask a coworker, and the assistant will pull the data, build the chart, or write the draft for you. When in doubt, just ask it "what can you do?"') }}</p>
        </div>
    </div>

    <p class="text-xs text-amber-600">{{ __('The AI Assistant is an Enterprise feature, and an admin can switch it on or off (and toggle the in-app copilot vs. the public portal bot) under Settings → AI.') }}</p>

    <h3 class="mt-6 text-base font-semibold text-gray-900">{{ __('Opening it & the basics') }}</h3>
    <ul class="mt-2 list-disc pl-5 space-y-1">
        <li>{{ __('Click the sparkle bubble in the bottom-right to open the chat. Click it again (or the ✕) to tuck it away.') }}</li>
        <li><strong>{{ __('Shift + Enter') }}</strong> {{ __('adds a new line; plain') }} <strong>{{ __('Enter') }}</strong> {{ __('sends your message.') }}</li>
    ...
    <h3 class="mt-6 text-base font-semibold text-gray-900">{{ __('What to ask — a cheat sheet') }}</h3>
    ...
    <h4 class="mt-5 flex items-center gap-2 text-sm font-semibold text-gray-900">
        <span class="text-indigo-600">①</span> {{ __('Find & filter your tickets') }}
    </h4>
    <p class="text-xs text-gray-500">{{ __('Let the assistant dig through the queue so you do not have to click filters.') }}</p>
    ...
    <h4 class="mt-5 flex items-center gap-2 text-sm font-semibold text-gray-900">
        <span class="text-indigo-600">②</span> {{ __('See the numbers — with charts') }}
    </h4>
    <p class="text-xs text-gray-500">{{ __('Ask for a "chart", "graph", "pie", or "trend" and the assistant draws it right in the chat. It picks a bar, pie, or line chart to fit the data.') }}</p>
    <div class="not-prose my-3 grid gap-2 sm:grid-cols-2">
        @foreach([
    ...
    <h4 class="mt-5 flex items-center gap-2 text-sm font-semibold text-gray-900">
        <span class="text-indigo-600">③</span> {{ __('Resolve problems faster') }}
    </h4>
    <p class="text-xs text-gray-500">{{ __('The assistant learns from every ticket your team closes. Describe a problem and it will surface how similar issues were solved before — then help you write the answer.') }}</p>
    <div class="not-prose my-3 grid gap-2 sm:grid-cols-2">
        @foreach([
    ...
    <h4 class="mt-5 flex items-center gap-2 text-sm font-semibold text-gray-900">
        <span class="text-indigo-600">④</span> {{ __('Drop in a screenshot or document') }}
    </h4>
    <p class="text-xs text-gray-500">{{ __('Use the paperclip to attach an image, PDF, or text file, then ask about it. The assistant can read error screenshots and skim documents.') }}</p>
    <div class="not-prose my-3 grid gap-2 sm:grid-cols-2">
        @foreach([
    ...
    <h4 class="mt-5 flex items-center gap-2 text-sm font-semibold text-gray-900">
        <span class="text-indigo-600">⑤</span> {{ __('Learn the app & look things up') }}
    </h4>
    <p class="text-xs text-gray-500">{{ __('It knows how this system works, and can reach the web for current, general questions when you need them.') }}</p>
    <div class="not-prose my-3 grid gap-2 sm:grid-cols-2">
        @foreach([
    ...
    <h3 class="mt-6 text-base font-semibold text-gray-900">{{ __('Get better answers — a few habits') }}</h3>
    <ul class="mt-2 list-disc pl-5 space-y-1">
        <li><strong>{{ __('Be specific.') }}</strong> {{ __('"High-priority open tickets for Acme this week" beats "show me tickets".') }}</li>
        <li><strong>{{ __('Build on the last answer.') }}</strong> {{ __('Say "now just the unassigned ones" or "make that a pie chart" instead of starting over.') }}</li>
        <li><strong>{{ __('Tell it the format you want.') }}</strong> {{ __('"In a table", "as three bullets", "a short reply I can send the client".') }}</li>
        <li><strong>{{ __('Ask it to show its work.') }}</strong> {{ __('"Which tickets did you count?" — it can list them.') }}</li>
    </ul>

    <h3 class="mt-6 text-base font-semibold text-gray-900">{{ __('Good to know (the guardrails)') }}</h3>
    <ul class="mt-2 list-disc pl-5 space-y-1">
    ...
    <p class="mt-6 text-sm text-gray-600">{{ __('That is it — open the sparkle button and try one of the prompts above. The fastest way to learn what your AI teammate can do is to ask it.') }}</p>
</div>
