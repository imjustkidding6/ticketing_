<div class="prose prose-sm max-w-none text-gray-700">
    ...
    <ul class="mt-2 list-disc pl-5 space-y-1">
        ...
        <li><strong>{{ __('In Progress') }}</strong> — {{ __('The agent is actively working on it. Moving to this status records the first-response time used for SLA.') }}</li>
        <li><strong>{{ __('On Hold') }}</strong> — {{ __('Paused, usually waiting on the client or a third party. Hold time is excluded from SLA calculations.') }}</li>
        <li><strong>{{ __('Closed') }}</strong> — {{ __('The issue has been resolved and the ticket is complete.') }}</li>
        <li><strong>{{ __('Cancelled') }}</strong> — {{ __('Closed without resolution, for example a duplicate or a false alarm.') }}</li>
    </ul>

    <h3 class="mt-6 text-base font-semibold text-gray-900">{{ __('Assigning & Self-Assigning') }}</h3>
    ...
    <h3 class="mt-6 text-base font-semibold text-gray-900">{{ __('Replies, Internal Notes & Attachments') }}</h3>
    ...
    <p class="text-xs text-amber-600">{{ __('Note: Attachments are available on Business and Enterprise plans. Canned responses (saved reply templates) are an Enterprise feature.') }}</p>
    ...
    <h3 class="mt-6 text-base font-semibold text-gray-900">{{ __('Hold Time') }}</h3>
    ...
    <h3 class="mt-6 text-base font-semibold text-gray-900">{{ __('Closing, Cancelling & False Alarms') }}</h3>
    ...
    <h3 class="mt-6 text-base font-semibold text-gray-900">{{ __('Reopening Tickets') }}</h3>
    ...
    <h3 class="mt-6 text-base font-semibold text-gray-900">{{ __('Merging Duplicates') }}</h3>
    ...
    <h3 class="mt-6 text-base font-semibold text-gray-900">{{ __('Spam & Activity History') }}</h3>
    ...
    <p class="text-xs text-amber-600">{{ __('Note: Mark as spam and ticket activity history (audit logs) are available on Business and Enterprise plans.') }}</p>
</div>
