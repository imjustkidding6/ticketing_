<div class="prose prose-sm max-w-none text-gray-700">
    ...
    <h3 class="mt-6 text-base font-semibold text-gray-900">{{ __('2. Review Your Departments') }}</h3>
    ...
    <p class="text-xs text-amber-600">{{ __('Note: Adding, renaming, and removing departments is an Enterprise feature. Starter and Business workspaces use the default department set.') }}</p>
    ...
    <h3 class="mt-6 text-base font-semibold text-gray-900">{{ __('4. Add Products (Optional)') }}</h3>
    ...
    <h3 class="mt-6 text-base font-semibold text-gray-900">{{ __('5. Invite Your Team') }}</h3>
    ...
    <ul class="mt-2 list-disc pl-5 space-y-1">
        <li><strong>{{ __('Agent') }}</strong> — {{ __('Works assigned tickets: updates status, adds replies and internal notes, and completes tasks.') }}</li>
        <li><strong>{{ __('Manager') }}</strong> — {{ __('Everything an agent can do, plus assigning tickets to others, managing clients, and viewing reports.') }}</li>
        <li><strong>{{ __('Admin') }}</strong> — {{ __('Full access to settings, roles, departments, SLA policies, and every management feature.') }}</li>
    </ul>
    <p class="text-xs text-amber-600">{{ __('Note: The number of users you can add is limited by your license seat count. On the Enterprise plan you can also define custom roles with their own permission sets.') }}</p>

    <h3 class="mt-6 text-base font-semibold text-gray-900">{{ __('6. Define Your SLA Policies') }}</h3>
    ...
    <p class="text-xs text-amber-600">{{ __('Note: When SLA management is enabled, a matching SLA policy must exist for a ticket\'s client tier and priority. Seeding the defaults covers every combination so ticket creation is never blocked.') }}</p>

    <h3 class="mt-6 text-base font-semibold text-gray-900">{{ __('7. Create Your First Ticket') }}</h3>
